...
def upgrade() -> None:
    ...
    with op.batch_alter_table("organization_integration_configs", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column(
                "whatsapp_api_version", sa.String(length=20), nullable=False, server_default="v21.0"
            )
        )
        batch_op.add_column(sa.Column("whatsapp_unauthorized_reply", sa.Text(), nullable=True))

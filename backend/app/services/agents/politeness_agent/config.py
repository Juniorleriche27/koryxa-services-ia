...
class PolitenessConfig(BaseModel):
    ...
    add_signoff: bool = Field(default=False, description="Add polite signoff at the end of the response")
    soften_no_results: bool = Field(default=True, description="Replace dry 'Not found' with diplomatic polite responses")
    ...
    def format_text(self, text: str) -> str:
        ...
        res = text.replace("{user_name}", u_name).replace("{company_name}", c_name).replace("{assistant_name}", a_name)
        import re

...
export default function PublicHeader() {
  ...
  return <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl supports-[backdrop-filter]:bg-white/85">
    <div className="mx-auto flex min-h-[72px] w-full max-w-[var(--marketing-max-w)] items-center gap-3 px-4 sm:px-6 lg:px-8">
      <Link href="/" className="flex min-h-11 shrink-0 items-center gap-3 rounded-xl focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-cyan-200" onClick={() => setOpen(false)} aria-label="KORYXA Service IA & Web — Accueil">
        <BrandLogo className="h-10 w-10 rounded-2xl" />
        <span className="leading-none"><strong className="block text-lg tracking-[-0.04em] text-slate-950">KORYXA</strong><small className="mt-1 block text-[10px] font-bold uppercase tracking-[0.16em] text-cyan-700">Service IA & Web</small></span>
      </Link>
      ...
      <Link href="/demarrer-un-projet" className="ml-auto hidden min-h-11 items-center justify-center rounded-full bg-cyan-400 px-5 text-sm font-bold text-slate-950 shadow-[0_14px_32px_rgba(55,211,138,0.22)] transition hover:bg-cyan-300 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-cyan-200 lg:inline-flex">Démarrer un projet</Link>
      <button onClick={() => setOpen(!open)} className="ml-auto inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-800 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-cyan-200 lg:hidden" aria-expanded={open} aria-controls="mobile-navigation" aria-label={open ? "Fermer le menu" : "Ouvrir le menu"}>{open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button>
    </div>
    {open ? <div id="mobile-navigation" className="fixed inset-x-0 top-[73px] max-h-[calc(100dvh-73px)] overflow-y-auto overscroll-contain border-t border-slate-200 bg-white px-4 py-4 shadow-2xl lg:hidden"><nav className="mx-auto max-w-[var(--marketing-max-w)]" aria-label="Navigation mobile"><button onClick={() => setServicesOpen(!servicesOpen)} aria-expanded={servicesOpen} aria-controls="mobile-services-menu" className="flex min-h-12 w-full items-center justify-between rounded-2xl bg-slate-50 px-4 py-3.5 text-left font-bold text-slate-950 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-cyan-200">Services <ChevronDown className={`h-4 w-4 transition ${servicesOpen ? "rotate-180" : ""}`} /></button>{servicesOpen ? <div id="mobile-services-menu" className="mt-2 grid gap-2">{pillars.map((pillar) => <Link onClick={() => setOpen(false)} key={pillar.slug} href={`/services/${pillar.slug}`} className="flex min-h-12 items-center rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-cyan-200">{pillar.title}</Link>)}</div> : null}<div className="mt-2 grid gap-1">{navItems.filter(i => i.href !== "/services").map(item => <Link onClick={() => setOpen(false)} key={item.href} href={item.href} className={`flex min-h-12 items-center rounded-2xl px-4 py-3.5 font-semibold focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-cyan-200 ${active(item.href) ? "bg-slate-950 text-white" : "text-slate-700"}`}>{item.label}</Link>)}</div><Link onClick={() => setOpen(false)} href="/demarrer-un-projet" className="mt-4 inline-flex min-h-12 w-full items-center justify-center rounded-2xl bg-cyan-400 font-bold text-slate-950 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-cyan-200">Démarrer un projet</Link></nav></div> : null}
  </header>;
}
